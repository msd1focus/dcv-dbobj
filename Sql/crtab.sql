...
DROP TABLE dcv.sec_dept CASCADE CONSTRAINTS;

DROP TABLE dcv.sec_user CASCADE CONSTRAINTS;

DROP TABLE dcv.wf_node CASCADE CONSTRAINTS;

DROP TABLE dcv.wf_node_route CASCADE CONSTRAINTS;
...
COMMENT ON COLUMN dcv.dcv_request.dcv_status IS
...
COMMENT ON COLUMN dcv.sec_dept.dept_code IS
    'DISTRI, SALES, TC, TAX, PROMO, AP, AR, CASSIER';

COMMENT ON COLUMN dcv.sec_dept.sla1 IS
    'sla1 : target berapa hari proses harus selesai diproses';

COMMENT ON COLUMN dcv.sec_dept.sla2 IS
    'cadangan';

COMMENT ON COLUMN dcv.sec_dept.role_parent IS
    'hirarchy';

ALTER TABLE dcv.sec_dept ADD CONSTRAINT role_pk PRIMARY KEY ( id );
...
COMMENT ON COLUMN dcv.sec_user.sp_assign2 IS
    'Role (untuk user pppc)';

ALTER TABLE dcv.sec_user ADD CONSTRAINT user_pk PRIMARY KEY ( id );

ALTER TABLE dcv.sec_user ADD CONSTRAINT sec_user_username_un UNIQUE ( username );
...
COMMENT ON COLUMN dcv.wf_node.assigned_users IS
...
COMMENT ON COLUMN dcv.wf_node_route.pilihan IS
    '1. Approve1
2. Approve2
3. Return/Back
4. Terminate/Cancel';

COMMENT ON COLUMN dcv.wf_node_route.pangkat IS
    'rank tertentu yg bisa lakukan action ini';

ALTER TABLE dcv.wf_node_route ADD CONSTRAINT node_option_pk PRIMARY KEY ( id );
...
COMMENT ON COLUMN dcv.wf_task.return_task IS
    'apakah merupakan return task ?';

COMMENT ON COLUMN dcv.wf_task.sla IS
    'running sla tiap hari';

CREATE INDEX dcv.wf_task_dcvno_idx ON
    dcv.wf_task (
        no_dcv
    ASC,
        nodecode
...
CREATE INDEX dcv.wf_task_type_idx ON
    dcv.wf_task (
        task_type
    ASC,
        progress_status
...
ALTER TABLE dcv.wf_task_merge ADD CONSTRAINT wf_task_merge_pk PRIMARY KEY ( id );
...
ALTER TABLE dcv.wf_task_merge
    ADD CONSTRAINT task_multiple_task_fk FOREIGN KEY ( task_id )
