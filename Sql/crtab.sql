...
DROP TABLE physical_doc CASCADE CONSTRAINTS;
...
CREATE TABLE dcv.dcv_files (
    ...
    physic_doc_keeper    VARCHAR2(25 CHAR),
    upload_time          DATE
);
...
CREATE TABLE physical_doc (
    doc_id        INTEGER NOT NULL,
    doc_type      VARCHAR2(25 CHAR) NOT NULL,
    dcv_id        INTEGER NOT NULL,
    dcv_code      VARCHAR2(30 CHAR),
    last_keeper   INTEGER
);

COMMENT ON COLUMN physical_doc.doc_type IS
    'kwitansi(asli)
kwitansi(copy)
po
gr
...
ALTER TABLE physical_doc ADD CONSTRAINT physical_doc_pk PRIMARY KEY ( doc_id );
...
ALTER TABLE doc_movement
    ADD CONSTRAINT doc_movement_physical_doc_fk FOREIGN KEY ( doc_id )
        REFERENCES physical_doc ( doc_id );
...
ALTER TABLE physical_doc
    ADD CONSTRAINT physical_doc_dcv_request_fk FOREIGN KEY ( dcv_id )
        REFERENCES dcv.dcv_request ( req_id );
