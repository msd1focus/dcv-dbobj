...
COMMENT ON COLUMN dcv.sec_dept.role_code IS
...
COMMENT ON COLUMN dcv.sec_dept.role_level IS
...
COMMENT ON COLUMN dcv.wf_task.merge_count IS
    'untuk tipe MERGE, jumlah route yg sudah complete. 0 = komplit';
...
CREATE TABLE dcv.wf_task_merge (
    ...
    prev_node     VARCHAR2(10 CHAR),
    process_dt    DATE,
    prime_route   CHAR(1 CHAR)
);

COMMENT ON COLUMN dcv.wf_task_merge.prime_route IS
    'Y/T';
...
ALTER TABLE dcv.sec_user
    ADD CONSTRAINT user_role_fk FOREIGN KEY ( department )
        REFERENCES dcv.sec_dept ( id )
            ON DELETE CASCADE;
